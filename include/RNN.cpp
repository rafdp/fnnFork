...
void RNN::ConnectLayerToLayer (int nSrcLayer, int nDstLayer, Initialization* initFunc, bool bTrainable)
{
    ...
    MarkLayerToLayerConnection (nSrcLayer, nDstLayer, bTrainable);
		//printf ("layer dims are %d %d\n", layers[nDstLayer]->GetSize(), layers[nSrcLayer]->GetSize()); 
    initFunc->Initialize (w[nDstLayer][nSrcLayer]);
	/*for (int i = 0; i < layers[nDstLayer]->GetSize(); i++)
	{
		for (int j = 0; j < layers[nSrcLayer]->GetSize(); j ++) printf ("%f ", w[nDstLayer][nSrcLayer][j][i]);
		printf ("\n");	
	}*/
}
